                                        ...
                                        {tournament.entries.map((entry) => (
                                            <div key={entry.id} className="flex items-center justify-between border-b pb-4 last:border-0 last:pb-0">
                                                <div className="flex items-center gap-3">
                                                    ...
                                                    <div>
                                                        <div className="font-medium">{entry.visit.player.name}</div>
                                                        <div className="text-xs text-muted-foreground">ID: {entry.visit.player.memberId}</div>
                                                    </div>
                                                </div>
                ...
                <div className="space-y-6">
                    <Card>
                        ...
                        <CardContent className="space-y-4">
                            <div>
                                <div className="text-sm text-muted-foreground mb-1">エントリー数</div>
                                <div className="flex items-center gap-2">
                                    <Users className="w-5 h-5 text-primary" />
                                    <span className="text-2xl font-bold">{tournament._count.entries}</span>
                                </div>
                            </div>

                            <div>
                                <div className="text-sm text-muted-foreground mb-1">プライズ総額</div>
                                <div className="flex items-center gap-2">
                                    <Trophy className="w-5 h-5 text-yellow-600" />
                                    <span className="text-2xl font-bold">
                                        {tournament.tournamentPrizes.reduce((sum, p) => sum + p.amount, 0).toLocaleString()}
                                    </span>
                                </div>
                            </div>
                            <div className="pt-2 space-y-2">
                                {tournament.tournamentPrizes.map((prize) => (
                                    <div key={prize.id} className="flex justify-between text-sm">
                                        ...
                                        <span className="text-muted-foreground">{prize.amount.toLocaleString()}</span>
                                    </div>
                                ))}
                            </div>
                        </CardContent>
                    ...
                    <Card>
                        ...
                        <CardContent className="space-y-4">
                            ...
                            <div className="flex justify-between text-sm">
                                ...
                                <span className="font-medium">{format(new Date(tournament.entryClosesAt), "HH:mm")}</span>
                            </div>
                        </CardContent>
                    </Card>
                </div>
