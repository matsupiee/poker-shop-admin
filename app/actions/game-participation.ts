...
import {
  InStoreRingChipEventType,
  WebCoinRingChipEventType,
...
export async function addTournamentEntry(
  ...
  if (!tournamentId)
    return { errors: { tournamentId: ["Tournament ID is required"] } };
  if (chipAmount < 0)
    return { errors: { _form: ["チップ量は0以上である必要があります"] } };
  if (paymentAmount < 0)
    return { errors: { _form: ["支払い額は0以上である必要があります"] } };
  ...
  try {
    ...
    if (!tournament) {
      return {
        errors: { tournamentId: ["指定されたトーナメントが見つかりません"] },
      };
    }
...
export async function addTournamentAddOn(
  ...
  if (!tournamentEntryId)
    return { errors: { _form: ["Entry ID is required"] } };
  if (chipAmount < 0)
    return { errors: { _form: ["チップ量は0以上である必要があります"] } };
  if (chargeAmount < 0)
    return { errors: { _form: ["支払い額は0以上である必要があります"] } };
    ...
    if (ringGameType === "WEB_COIN") {
      ...
      if (eventType === "WITHDRAW") {
        ...
        try {
          ...
          const errorMessage =
            error instanceof Error ? error.message : "引き出しに失敗しました";
          return {
            success: false,
            errors: {
              _form: [errorMessage],
...
export async function addRingGameChip(
  ...
  if (!ringGameEntryId)
    return { errors: { _form: ["Ring Game Entry ID is required"] } };
  if (amount < 0)
    return { errors: { _form: ["チップ量は0以上である必要があります"] } };
