...
async function getAvailableDeposits(playerId: string) {
    ...
    return deposits.filter((deposit) => {
        const usedAmount = deposit.inStoreChipDepositWithdrawAmounts.reduce((sum, wa) => sum + wa.amount, 0)
 ...
 * 店内チップを入金する
 */
export async function depositInStoreChip(playerId: string, amount: number, visitId?: string) {
 ...
 */
export async function withdrawInStoreChip(playerId: string, amount: number, chipEventId?: string) {
