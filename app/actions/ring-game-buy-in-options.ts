...
import { type InStoreRingBuyInOption, RingGameType } from '@/lib/generated/prisma/client';
import { revalidatePath } from 'next/cache';
import { auth } from '@/lib/better-auth/auth';
import { headers } from 'next/headers';

export type RingGameBuyInOption = InStoreRingBuyInOption & { ringGameType: RingGameType };
...
export async function getRingGameBuyInOptions() {
    ...
    const options = [
        ...inStoreOptions.map(o => ({ ...o, ringGameType: RingGameType.IN_STORE })),
    ];
...
export async function deleteRingGameBuyInOption(id: string, ringGameType: RingGameType) {
    try {
        await checkAuth();
        if (ringGameType === RingGameType.WEB_COIN) {
            throw new Error('WEB_COIN does not support buy-in options');
