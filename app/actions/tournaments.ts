...
export async function createTournament(
  ...
  if (hasBounty) {
    if (
      !bountyTicketCountStr ||
      isNaN(Number(bountyTicketCountStr)) ||
      Number(bountyTicketCountStr) < 1
    ) {
      errors._form = [
        ...(errors._form || []),
        "バウンティチケット枚数を正しく入力してください",
      ];
    } else {
    ...
    if (
      !bountyTotalAmountStr ||
      isNaN(Number(bountyTotalAmountStr)) ||
      Number(bountyTotalAmountStr) < 0
    ) {
      errors._form = [
        ...(errors._form || []),
        "バウンティ総額を正しく入力してください",
        ...
        for (let i = 1; i < prizes.length; i++) {
          if (prizes[i].amount > prizes[i - 1].amount) {
            return {
              errors: {
                _form: [
                  `${prizes[i].rank}位の賞金が${prizes[i - 1].rank}位より大きくなっています。`,
                ],
              },
            };
          }
    ...
    try {
      ...
      if (Array.isArray(parsed)) {
        chipEventOptions = parsed
          ...
          .filter(
            (p) => p.name && !isNaN(p.chipAmount) && !isNaN(p.chargeAmount)
          );
      }
    } catch (e) {
