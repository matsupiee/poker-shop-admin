...
async function getAvailableDeposits(playerId: string) {
    ...
    return deposits.filter((deposit) => {
        const usedAmount = deposit.webCoinDepositWithdrawAmounts.reduce((sum, wa) => sum + wa.amount, 0)
 ...
 * Webコインを入金する
 */
export async function depositWebCoin(playerId: string, amount: number, visitId?: string) {
...
export async function withdrawWebCoin(playerId: string, amount: number) {
    ...
    await prisma.$transaction(async (tx) => {
        // 1. 出金レコードの作成
        const withdraw = await tx.webCoinWithdraw.create({
            data: {
                playerId,
        ...
        // 未使用残高がある入金レコードを取得
        const deposits = await tx.webCoinDeposit.findMany({
            where: { playerId },
            include: {
                webCoinDepositWithdrawAmounts: true
            },
        ...
        for (const deposit of deposits) {
            ...
            if (availableAmount > 0) {
                const amountFromThisDeposit = Math.min(availableAmount, remainingToWithdraw)

                await tx.webCoinDepositWithdrawAmount.create({
                    data: {
