...
export function TournamentDialog({
  ...
  const [selectedTournamentId, setSelectedTournamentId] =
    React.useState<string>(defaultTournamentId);
  ...
  const availableOptions = React.useMemo(() => {
    ...
    if (!existingEntryId) {
      return currentTournament.tournamentChipEventOptions.filter(
        (opt) => opt.eventType === "ENTRY"
      );
    }
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (selectedOption.eventType === "ENTRY") {
        ...
        result = await addTournamentAddOn(existingEntryId, amount, payment);
      }
      ...
      <DialogContent className="sm:max-w-[425px]">
        ...
        <DialogFooter>
          <Button
            ...
            disabled={
              isSubmitting || !selectedTournamentId || !selectedOptionId
            }
          >
            {isSubmitting
              ? "処理中..."
              : selectedOption?.eventType === "ENTRY"
                ? isJoin
                  ? "参加する"
                  : "リエントリー"
                : "アドオン追加"}
