...
export function DashboardView({ month, stats }: DashboardViewProps) {
  ...
  const totalTournaments = stats.reduce(
    (acc, curr) => acc + curr.tournaments,
    0
  );
  const totalTournamentEntries = stats.reduce(
    (acc, curr) => acc + curr.tournamentEntries,
    0
  );
  const totalRingGameEntries = stats.reduce(
    (acc, curr) => acc + curr.ringGameEntries,
    0
  );
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
        <Card>
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            <CardTitle className="text-sm font-medium">合計来店人数</CardTitle>
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">
              {totalRingGameEntries.toLocaleString()}名
            </div>
          </CardContent>
        </Card>
        <Card>
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            <CardTitle className="text-sm font-medium">月間粗利</CardTitle>
