    ...
    <div className="container mx-auto py-10 space-y-8">
      <div className="flex items-center gap-4">
        ...
        <div>
          ...
          <p className="text-muted-foreground">会員ID: {player.memberId}</p>
        </div>
      </div>
                  ...
                  <TableBody>
                    {visits.map((visit) => (
                      <TableRow key={visit.id}>
                        ...
                        <TableCell>
                          {visit.entranceFee?.toLocaleString() ?? "-"} 円
                        </TableCell>
                        <TableCell>
                          {visit.foodFee?.toLocaleString() ?? "-"} 円
                        </TableCell>
                      </TableRow>
                    ))}
                    {visits.length === 0 && (
                  ...
                  <TableBody>
                    {webCoinLogs.map((log) => (
                      <TableRow key={log.id}>
                        ...
                        <TableCell>
                          {log.type === "deposit" ? "預入" : "引出"}
                        </TableCell>
                        <TableCell
                          className={`text-right font-bold ${log.type === "deposit" ? "text-green-600" : "text-red-600"}`}
                        >
                          {log.type === "deposit" ? "+" : "-"}
