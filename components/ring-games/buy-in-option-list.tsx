...
export function BuyInOptionList({ options }: BuyInOptionListProps) {
  ...
  const [selectedOption, setSelectedOption] =
    useState<RingGameBuyInOption | null>(null);
  ...
  const handleDelete = async (
    id: string,
    ringGameType: (typeof RingGameType)[keyof typeof RingGameType]
  ) => {
            ...
            {options.map((option) => (
              <TableRow key={option.id}>
                ...
                <TableCell className="text-right">
                  {formatNumber(option.chipAmount)}
                </TableCell>
                <TableCell className="text-right">
                ...
                <TableCell>
                  <div className="flex gap-2 justify-end">
                    ...
                    <Button
                      ...
                      onClick={() =>
                        handleDelete(option.id, option.ringGameType)
                      }
                    >
