...
import {
  createDealerShift,
  updateDealerShift,
} from "@/app/actions/dealer-shifts";
...
export function DealerShiftDialog({
  staffList,
  deskList,
  initialData,
  children,
  onSuccess,
}: DealerShiftDialogProps) {
...
function DealerShiftForm({
  ...
  const updateAction = initialData
    ? updateDealerShift.bind(null, initialData.id)
    : null;
  ...
  const [ringGameType, setRingGameType] = useState<string>(
    initialData?.ringGameType || RingGameType.WEB_COIN
  );
  ...
  const initialDate = initialData
    ...
    : today;
  const initialStartTime = initialData
    ? format(new Date(initialData.startedAt), "HH:mm")
    : "19:00";
  const initialEndTime =
    initialData && initialData.endedAt
          ...
          <div className="col-span-3">
            ...
            >
              ...
              <SelectContent>
                ...
                <SelectItem value={RingGameType.IN_STORE}>
                  店内チップ
                </SelectItem>
              </SelectContent>
            </Select>
