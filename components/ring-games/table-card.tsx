...
import {
  sitDownAtTable,
  standUpFromTable,
} from "@/app/actions/ring-game-tables";
...
export function TableCard({ desk, staffList }: TableCardProps) {
  ...
  return (
    <Card
      className={`relative overflow-hidden transition-all hover:shadow-md ${isActive ? "border-primary/50 bg-primary/5" : "border-dashed"}`}
    >
      <CardHeader className="pb-2">
        <CardTitle className="flex items-center justify-between">
          <span className="text-lg font-bold">{desk.name}</span>
          <Armchair
            className={`h-5 w-5 ${isActive ? "text-primary" : "text-muted-foreground/30"}`}
          />
        </CardTitle>
      </CardHeader>
      <CardContent>
            ...
            <div className="space-y-4">
              <div className="space-y-1">
                ...
                <p className="text-xs text-muted-foreground">
                  Started{" "}
                  {formatDistanceToNow(new Date(desk.activeShift!.startedAt), {
                    addSuffix: true,
                    locale: ja,
                  })}
                </p>
