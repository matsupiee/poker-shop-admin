...
export function ChipEventOptionsForm({
  options,
  onChange,
}: ChipEventOptionsFormProps) {
  const addOption = () => {
    onChange([
      ...options,
    ...
    <div className="space-y-4 border-t pt-4">
      ...
      <div className="space-y-4 pr-2">
        ...
        {options.map((option, index) => (
          <div
            key={index}
            className="grid grid-cols-[120px_160px_160px_160px_24px] gap-2 items-start text-sm border p-2 rounded-md"
          >
            ...
            <div className="grid gap-1 relative">
              ...
              <Input
                ...
                onChange={(e) =>
                  updateOption(index, "chipAmount", Number(e.target.value))
                }
                className="h-8"
                placeholder="0"
              />
            ...
            <div className="grid gap-1 relative">
              ...
              <Input
                ...
                onChange={(e) =>
                  updateOption(index, "chargeAmount", Number(e.target.value))
                }
                className="h-8 pl-6"
                placeholder="0"
              />
              <span className="absolute left-2 top-[26px] text-muted-foreground text-xs">
                ¥
              </span>
            </div>
            <Button
      ...
      <input
        ...
        value={JSON.stringify(
          options.filter((o) => o.chipAmount > 0 || o.chargeAmount > 0)
        )}
      />
    </div>
  );
