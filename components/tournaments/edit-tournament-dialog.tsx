...
export function EditTournamentDialog({ tournament, onTournamentUpdated, children }: EditTournamentDialogProps) {
    ...
    }, [state.success, onTournamentUpdated])
    ...
    const isStarted = new Date() > startAtDate

    if (isStarted) {
        // If started, do not allow editing (or render disabled trigger)
        // If children are provided (trigger), render it but maybe disabled? 
        // Or if it's our own button, disable it.
        // Actually, let's render it disabled or show a tooltip saying why.
        // For simplicity, we just won't render the trigger action if it's started? 
        // User said "后から修正できないように", so hiding the edit capability is fine.
        return null
    }
