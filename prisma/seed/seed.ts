...
const seedStaff = async () => {
    ...
    for (const data of staffDataList) {
        const existingStaff = await prisma.staff.findUnique({ where: { email: data.email } });
...
async function main() {
    ...
    if (!await prisma.tournament.findFirst({ where: { name: tournamentName, startAt: today } })) {
    ...
    if (!await prisma.tournament.findFirst({ where: { name: tomorrowName, startAt: tomorrow } })) {
